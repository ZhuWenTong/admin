<template>
    <a-modal v-model="visible" title="编辑" ok-text="确定" cancel-text="取消" @cancel="cancel" @ok="ok" width="400px">
        <a-form-model ref="ruleForm" :model="model" :rules="rules" :label-col="{span: 6}" :wrapper-col="{span: 18}" labelAlign="right">
            ...
            <a-form-model-item has-feedback label="具体事由" prop="reasonId">
                <a-select placeholder="请选择具体事由" allowClear @change="selectReason" v-model="model.reasonId">
...
export default {
    ...
    data () {
        return {
            ...
            rules: {
                ...
                reasonId: [
                    {required: true, message: '请选择具体事由', trigger: 'blur'}
                ],
                money: [
    ...
    methods: {
        ...
        selectReason (val) {
            let obj = this.reasonList.find(i => i.value === val)
            this.model.reason = obj ? obj.label : ''
        }
    },
    watch: {
        dialogVisible: {
